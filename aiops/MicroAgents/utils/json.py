...
def parse_json_markdown(
    ...
    try:
        ...
        match = re.search(r'(?:```json|```|json)(.*)', json_string, re.DOTALL)
        ...
        if match is None:
            if "**cause" in json_string.lower()  and "**Module**" in json_string:
                res = {}
                res["CAUSE"] = []
                flag = False
                for line in json_string.split("\n"):
                    if "**cause" in line.lower():
                        flag = True
                    if "**Module**" in line and flag:
                        start = True
                        tmp = {}
                        tmp["Module"] = line.split("**Module**:")[1].strip()
                    if '**Abnormal Type**' in line and flag and start:
                        tmp["Abnormal Type"] = line.split("**Abnormal Type**:")[1].strip()
                    if '**Possiblity**' in line and flag and start:
                        tmp["Possiblity"] = line.split("**Possiblity**:")[1].strip()
                        res["CAUSE"].append(tmp)
                        tmp = {}
                        start = False
            return res
        if match is None:
            ...
            match = re.search(r'(\[.*\])', json_string, re.DOTALL)
            if match is not None:
                json_str =  f'"{{CAUSE":{match.group(1)}}}'       
        ...
        if match is None:
            ...
            # json_str = match.group(1)
            if "CAUSE" not in json_str:
                json_str = f'"{{CAUSE":[{match.group(1)}]}}'
                
        if '```' in json_str:
            json_str = json_str.split('```')[0]
        json_str = rectify(json_str)
