# 测试中：使用多尺度的patch对于repeat后的信息进行优化 ⛽️
...
class AnomalyAttention(nn.Module):
    ...
    def forward(self, queries_patch_size, queries_patch_num, keys_patch_size, keys_patch_num, values, patch_index, attn_mask):
        ...
        if self.mask_flag:
            ...
            scores_patch_size.masked_fill_(attn_mask.mask, -np.inf)
        ...
        if self.mask_flag:
            if attn_mask is None:
                attn_mask = TriangularCausalMask(B, L, device=queries.device)
            scores_patch_num.masked_fill_(attn_mask.mask, -np.inf)
        ...
        if self.output_attention:
            ...
            # return series_patch_num, series_patch_size
        else:
            return (None)
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, patch_size, channel, n_heads, win_size, d_keys=None, d_values=None):
        ...
        self.patch_size_query_projection = nn.Linear(d_model, d_keys * n_heads)
        self.patch_size_key_projection = nn.Linear(d_model, d_keys * n_heads)
        self.patch_num_query_projection = nn.Linear(d_model, d_keys * n_heads)
        self.patch_num_key_projection = nn.Linear(d_model, d_keys * n_heads)
    ...
    def forward(self, x_patch_size, x_patch_num, x_ori, patch_index, attn_mask):
        ...
        queries_patch_size = self.patch_size_query_projection(queries_patch_size).view(B, L, H, -1) 
        keys_patch_size = self.patch_size_key_projection(keys_patch_size).view(B, L, H, -1) 

        # patch_num
        B, L, M = x_patch_num.shape
        queries_patch_num, keys_patch_num = x_patch_num, x_patch_num
        queries_patch_num = self.patch_size_query_projection(queries_patch_num).view(B, L, H, -1) 
        keys_patch_num = self.patch_size_key_projection(keys_patch_num).view(B, L, H, -1)
