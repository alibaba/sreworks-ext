...
class AD_MODEL(object):
    ...
    def save_weight_GD(self):
        ...
        torch.save(self.G.state_dict(), os.path.join(save_dir, self.model+'_'+str(self.opt.dataroot.split('/')[-1]) + '_G.pkl'))
        torch.save(self.D.state_dict(), os.path.join(save_dir, self.model+'_'+str(self.opt.dataroot.split('/')[-1]) + '_D.pkl'))

    def load(self):
        save_dir = os.path.join(self.outf, self.model, self.dataset,"model")

        self.G.load_state_dict(torch.load(os.path.join(save_dir, self.model+'_'+str(self.opt.dataroot.split('/')[-1]) + '_G.pkl')))
        self.D.load_state_dict(torch.load(os.path.join(save_dir, self.model+'_'+str(self.opt.dataroot.split('/')[-1]) + '_D.pkl')))
