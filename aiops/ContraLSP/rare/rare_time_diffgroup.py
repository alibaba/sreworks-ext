...
def run_experiment(
    ...
    N_features: int = 50,
    N_select: int = 5,
    save_dir: str = "experiments/results/rare_time_diffgroup/",
):
    """Run experiment.
    ...
    t_rand2 = np.random.randint(low=0, high=T - N_select)
    true_saliency[:N_ex//2, t_rand: t_rand + N_select, : int(2 * N_features / 4)] = 1
    true_saliency[N_ex//2:, t_rand2: t_rand2 + N_select, int(N_features / 4): int(3 * N_features / 4)] = 1
    with open(os.path.join(save_dir, f"true_saliency_{cv}.pkl"), "wb") as file:
        pkl.dump(true_saliency, file)
    print("==============The mean of true_saliency is", true_saliency.sum()/N_ex, "==============\n" + 100 * "=")

    # The white box only depends on the truly salient features
    def f(input):
        bs, t, n = input.shape
        output = torch.zeros((bs, t, 1), device=input.device)
        output[:N_ex//2, t_rand : t_rand + N_select, :] = (input[
            :N_ex//2, t_rand : t_rand + N_select, : int(2 * N_features / 4)
        ].sum(dim=-1).unsqueeze(-1)) ** 2
        output[N_ex//2:, t_rand2 : t_rand2 + N_select, :] = (input[
            N_ex//2:, t_rand2 : t_rand2 + N_select, int(N_features / 4): int(3 * N_features / 4)
        ] ** 2).sum(dim=-1).unsqueeze(-1)
        return output
        ...
        for k in range(N_ex):  # We compute the attribution for each individual time series
            ...
            if k<N_ex//2:
                def f(input):
                    ...
                    output[t_rand: t_rand + N_select, : int(2 * N_features / 4)] = input[
                          t_rand: t_rand + N_select, : int(2 * N_features / 4)
                          ]
                ...
                def f(input):
                    output = torch.zeros(input.shape, device=input.device)
                    output[t_rand2: t_rand2 + N_select, int(N_features / 4): int(3 * N_features / 4)] = input[
                          t_rand2: t_rand2 + N_select, int(N_features / 4): int(3 * N_features / 4)
                          ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default="./results/rare_time_diffgroup/", type=str)
