...
def run_experiment(
    ...
    true_saliency[:, t_rand: t_rand + N_select, int(N_features / 4): int(3 * N_features / 4)] = 1
    ...
    # Initialize the saliency tensors and black-box function
    def f(input):
        output = torch.zeros(input.shape, device=input.device)
        output[t_rand : t_rand + N_select, int(N_features / 4) : int(3 * N_features / 4)] = input[
            t_rand : t_rand + N_select, int(N_features / 4) : int(3 * N_features / 4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default="./results/rare_time/", type=str)
