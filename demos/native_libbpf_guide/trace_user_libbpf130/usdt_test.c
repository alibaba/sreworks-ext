...
int main(int argc, char *argv[])
{
    ...
    bpf_prog1 = bpf_object__find_program_by_name(bpf_obj, "usdt_probe1");
    bpf_link1 = bpf_program__attach_usdt(bpf_prog1, -1, "/usr/bin/umark", "groupa", "probe1", NULL);
    if (libbpf_get_error(bpf_link1)) {
        printf("ERROR: failed to attach_usdt1: '%s'\n", strerror(errno));
        return 2;
    }

    bpf_prog2 = bpf_object__find_program_by_name(bpf_obj, "usdt_probe2");
    bpf_link2 = bpf_program__attach(bpf_prog2);
    if (libbpf_get_error(bpf_link2)) {
        printf("ERROR: failed to attach_usdt2: '%s'\n", strerror(errno));
        return 2;
    }
